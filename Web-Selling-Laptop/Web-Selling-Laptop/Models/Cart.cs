...
namespace Web_Selling_Laptop.Models
{
    ...
    public class Cart
    {
        ...
        public void Add_Product_Cart(Laptop _pro, int _quan = 1)
        {
            var item = Items.FirstOrDefault(s => s._laptop.LaptopId == _pro.LaptopId);
            if (item == null)
                items.Add(new CartItem { _laptop = _pro, _quantity = _quan });
            else
        ...
        {
            var total = items.Sum(s => s._quantity * s._laptop.Price);
            return (decimal)total;
        }
        // Phương thức cập nhật số lượng khi khách hàng chọn SP mua thêm
        public void Update_quantity(int id, int _new_quan)
        {
            var item = items.Find(s => s._laptop.LaptopId == id);
            if (item != null)
        ...
        {
            items.RemoveAll(s => s._laptop.LaptopId == id);
        }
